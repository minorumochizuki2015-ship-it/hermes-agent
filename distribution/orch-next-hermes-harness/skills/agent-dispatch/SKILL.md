...
Resolve both CMD roles from `controls/routing-table.json`; do not pass either
app-bound CMD role or `surface_kind=app_bound_cmd` to `critical_thread_route.v1`.
The critical route is Codex-internal only for planner, integrator, and audit
classes. A typed `BLOCKED_FOR_CRITICAL_ROUTE_APP_BOUND_CMD` result is advisory
for route selection: preserve the current action and continue through the
admitted `cmd_primary` or `cmd_alternate` surface.

## Critical route adaptation

Before creating a Codex-internal planning, integration, authority,
approval-handoff, or final-audit thread, run the executable
`critical_thread_route.v1` pre-create consumer. The packet binds the role
class, surface kind, model-route authority, required and actual model/effort,
final-verdict authority, paid flag, one-writer target, child bootstrap,
rollback, and expiry. Pass the returned model and thinking arguments
explicitly; app-bound CMD work never enters this route.
