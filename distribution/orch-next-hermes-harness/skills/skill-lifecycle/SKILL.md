...
version: "1.0.0"
description: Use for skill distribution changes; not skill usage.
...
Own source health, recursive distribution, installed-state separation, collision handling, quarantine, retirement, rollback, and observed firing checks.
...
## M-13 drilled rollback and fixture retirement

Marker: `FABLE5_M13_DRILLED_ROLLBACK_AND_RETIREMENT`.

Every card that changes runtime state binds an executable reversal, the exact
rollback identity, and at least one sanitized drill receipt for the train
before promotion. “Revert somehow” is not executable. Retirement additionally
requires a red-first zero-consumer enumerator, reversible archive, and rollback
drill. Controls and fixtures carry an explicit expiry or demotion condition so
they can retire when they add no decision delta; three fresh eligible phases
is a calibration-pending candidate, never an automatic deletion rule.

`NF-M13` rejects runtime-state-changing work without executable rollback or a
drill receipt using
`BLOCKED_FOR_FABLE5_NF_M13_RUNTIME_ROLLBACK_UNDRILLED`. Source presence or
validator PASS is not a drill, installed adoption, runtime acceptance, or
retirement proof.
...
2. Hermes operates the lifecycle; protected promotion and claim decisions consume typed Maestro authority.
