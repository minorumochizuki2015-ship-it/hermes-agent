---
name: action-first-operator-communication
version: "1.0.0"
description: Use for result-first ops updates; not deep explanations.
author: ORCH-Next contributors and Hermes Agent
license: MIT; see references/UPSTREAM_NOTICE.md
metadata:
  hermes:
    category: orch-next
...
# Action First Operator Communication Skill

Format progress, failures, blockers, results, and closeout as a compact state delta.
...
2. Write the outcome first, retain exact non-claims, and end with the next Hermes action or a real protected transition.
...
The adopted result-first concepts and upstream license are preserved in
`references/UPSTREAM_NOTICE.md`. The optional interface hint lives in
`agents/openai.yaml`; it does not change Hermes ownership or authority.
