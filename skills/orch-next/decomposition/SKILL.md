...
name: decomposition
version: "1.0.0"
description: Use for Grand Goal slicing; not one-step tasks.
...
Turn a Grand Goal into ordered slices with user-visible deltas, dependencies, negative checks, and claim boundaries.
...
## M-01 verify-then-plan fact base

Marker: `FABLE5_M01_VERIFY_THEN_PLAN_DECOMPOSITION`.

Before slicing work, re-read every load-bearing current fact from its nominated
source and bind `fact_id`, `source_path`, `source_head`, `evidence_head`,
`confidence`, and `falsifier`. The declared evidence head must equal the head
that supplied the fact. Split that verified base into current-state,
premise-challenge, root-cause, option, and program layers with stable IDs before
producing the slice ladder. Stale evidence may remain as explicit
counterevidence, but no downstream claim may cite it as current.

`NF-M01` rejects the affected current-fact claim when a load-bearing fact comes
from a different source head than its declared evidence head with
`BLOCKED_FOR_FABLE5_NF_M01_STALE_EVIDENCE_HEAD`. Re-verification is a Claim
Check: it does not block disjoint safe Hermes work.
...
2. Select the next cause-changing slice; give support-only work zero product-progress credit.
