---
name: image-gen-lane-dispatch
version: "1.0.0"
description: Use for material visual assets; not decorative extras.
...
# Image Gen Lane Dispatch Skill

Create an isolated image task only when imagery materially carries product identity or interaction meaning.
...
2. Read and complete
   `references/six-block-image-gen-template.md` before dispatch. Bind the
   exact asset purpose, dimensions, accepted reference, isolated staging
   destination, must/must-not constraints, and comparison axes.
3. Use the available image-generation route, preserve selected-versus-rejected
   candidates, inspect every returned candidate with `vision_analyze`, and
   keep generated assets out of production until accepted.
