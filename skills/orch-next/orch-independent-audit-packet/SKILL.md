---
name: orch-independent-audit-packet
version: "1.0.0"
description: Use for independent fixed-revision audit; not writing.
...
# Orch Independent Audit Packet Skill

Bind a read-only auditor to a fixed source revision, goal, requirements, diff, checks, negative fixtures, and non-claims.
...
2. Load `references/delegate-task-contract.json` with `read_file`. Bind the
   exact repository, worktree, fixed source revision, Grand Goal, requirements,
   diff, checks, negative fixtures, and non-claims as the read manifest.
3. Call `delegate_task` in batch mode with exactly one task item containing
   role `leaf`, `capability_profile="read_only_audit"`, the exact
   `target_revision`, and a bounded `timeout_seconds`. After resolving all
   toolsets, the Hermes runtime publishes only the exact read-only allowlist
   and reapplies it after registry/schema refresh and at Tool Search unwrap.
   The runtime removes terminal/process, `execute_code`, `write_file`, `patch`,
   `skill_manage`, browser, delegation, and every unlisted future or overlay
   tool before launch. The write manifest is exactly `[]`. `target_revision`
   is validated and prompt-bound; it does not pin a checkout, so the controller
   must supply a worktree already fixed to that revision.
4. Require the Hermes-owned tool loop. If the effective runtime resolves to
   `codex_app_server`, ACP, `copilot-acp`, or another external tool loop not
   proven to traverse the exact Hermes allowlist, consume
   `capability_runtime_incompatible` with `status=unavailable`, `launch=false`,
   and `launch_count=0`. Do not retry through that runtime, silently downgrade
   it, or fall back to Maestro/Codex execution.
5. Require one structured return containing terminal class, fixed revision,
   findings, bounded verdict input, blockers, and non-claims. The audit cannot
   self-authorize or issue final user acceptance.
6. For an ended audit session, use a registered resume surface (`/resume` in
   the gateway or `hermes --resume` in the CLI), then at most one read-only
   `delegate_task` atomic successor. `session.reconnect` is not registered; if
   reconnect is required, return `hermes_reconnect_unavailable` with
   `launch=false`. Do not create a duplicate audit owner or use the user as a
   packet/session relay.
7. If the Hermes destination or required authority input is unavailable,
   return the typed failure with `launch=false`. Never launch Maestro or Codex
   as an audit fallback.
8. Use only native read-only Hermes tools such as `read_file`, `search_files`,
   `skill_view`, and `vision_analyze` when necessary.
...
- `hermes_runtime_unavailable`: the required Hermes operational surface is
  ...
  execution.
- `capability_runtime_incompatible`: the effective child tool loop does not
  traverse the exact Hermes read-only allowlist; return `status=unavailable`,
  `launch=false`, and `launch_count=0` without downgrade or fallback.
